...
// 함수형컴퍼넌트 : 렌더링 -> 함수호출 -> 컴퍼넌트 내부의 모든 변수가 초기화
// 대부분의 컴퍼넌트는 state, props가 변경이 되면 랜더링된다.

function calculate () { return "무거운 작업"; }

const havyCaculate = (number) => {
	console.log(`시간걸리는 계산작업`);
	for (let i = 0; i <= 99999999; i++){}
	return number + 100000;
}

const easyCalculate = (number) => {
	console.log(`쉬운 계산 작업`);
	return number + 1;
}
const Hook04_01useMemo = () => {

	const value = calculate(); // ajax, fetchm, axios 비동기처리
	// console.log(`무거운 작업 결과 = ${value}`);
