...
const Hook01_02useState = () => {
	...
	// 변수상태의 초기화 방법
	// const [users, setUsers] = useState(['손흥민', '김민재', '이강인']);
	// const [users, setUsers] = useState(fn_havyWork());    // 변수 상태가 변경할 때 마다 실행
	const [users, setUsers] = useState(() => fn_havyWork()); // 콜백함수는 초기화할 때 한번만 실행

	const handleTimeClick = () => {
		const time = new Date();
		// console.log(time.toLocaleDateString() + " " + time.toTimeString());
		setNow(time.toLocaleDateString() + " " + time.toTimeString());
	}
