...
/*
	...
	B. React.memo
			...
			4) props(값 or 객체)를 비교할 때는 얕은 비교를 한다.
	C. 고차함수(HOC)?
			고차함수(컴퍼넌트)는 함수를 가져와서 새로운 함수로 변환하는 함수를 말한다.
			부모가 랜더링될 때마다 자식에 props전달하는 경우에 자식의 props가 변동이 없음에도
			자식컴퍼넌트로 re-rendering이 된다. 이때 자식 컴퍼넌트를 전달해서 props check를 통해
			props가 변동이 없다면 랜더링을 하지않도록 하여 재사용하도록한다. 이를 통해 컴퍼넌트
			의 최적화를 유지할 수가 있다.
			주의할 점은 무분별하게 사용한다면 오히려 독이 된다.
			React.memo는 props변화에만 의존하는 최적화 방법이다. React.memo를 사용할 경우는 
...
const Hook08_01ReactMemo = () => {
	...
	return (
		<div style={{border: '2px solid navy', padding: '10px'}}>
			...
			<button onClick={increaseChildAge}>자식 나이 증가</button><br /><br />
